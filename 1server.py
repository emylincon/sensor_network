...
def convert_(x_list):
  if x_list[-1] < 60:
    return x_list, 'seconds'
  if x_list[-1] < (60*60):
    return [round(x/60, 3) for x  in x_list], "minutes"
  else:
    return [round(x_list/(60*60)) for x  in x_list], "hours"


def plotter(ax, data, key, name, col, x_axis):
    ax.grid(True)
    x_axis, metric = convert_(x_axis)
    ax.plot(x_axis, _mov_avg(data), col, linewidth=2, label='{}'.format(name))
    #ax.set_ylabel('Moving {}'.format(name))
    ax.set_xlabel(f'Time ({metric})')
    if name == 'Memory':
